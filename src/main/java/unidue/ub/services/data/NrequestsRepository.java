...
public interface NrequestsRepository  extends PagingAndSortingRepository<Nrequests, String> {
    ...
    @Query(value = "SELECT p.* FROM Nrequests p WHERE SUBSTRING(p.shelfmark,1,3) BETWEEN :startNotation AND :endNotation AND ratio >= :ratio AND nrequests >= :thresholdNrequests AND total_duration >= :duration AND date >= :startDate" +
            "", nativeQuery = true)
