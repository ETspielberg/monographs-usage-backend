...
public interface NrequestsRepository  extends PagingAndSortingRepository<Nrequests, String> {
    ...
    @Query(value = "SELECT p.* FROM Nrequests p WHERE SUBSTRING(p.shelfmark,1,3) BETWEEN :startNotation AND :endNotation AND ratio >= :ratio AND nrequests >= :thresholdNrequests AND total_duration >= :duration SELECT mab, shelfmark, MAX(ratio) AS ratio, " +
            "       MAX(nitems) AS nitems, MAX(nlendable) AS nlendable, MAX(nloans) AS nloans, MAX(nrequests) AS nrequests, title_id, MAX(total_duration) as total_duration " +
            "  FROM nrequests " +
            "  WHERE SUBSTRING(shelfmark,1,3) BETWEEN :startNotation AND :endNotation AND ratio >= :ratio AND nrequests >= :thresholdNrequests AND total_duration >= :duration AND date >= :startDate GROUP BY mab,shelfmark,title_id", nativeQuery = true)
